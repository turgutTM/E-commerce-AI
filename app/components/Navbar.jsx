...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await axios.get(`/api/shop-cart-products/${user._id}`);

        if (response.data && response.data.cartDetails) {
          const formattedCartItems = response.data.cartDetails.map((item) => ({
            _id: item.productId,
            name: item.name,
            imgURL: item.imgURL,
            price: item.price,
          ...
          setPrevCartLength(formattedCartItems.length);
        }
      } catch (error) {
          ...
          >
            {isLoading ? (
              <div className="animate-pulse">
                ...
                <span className="absolute -top-2 -right-2 bg-gray-300 text-transparent text-xs w-5 h-5 flex items-center justify-center rounded-full">
                  0
                </span>
              </div>
            ) : (
